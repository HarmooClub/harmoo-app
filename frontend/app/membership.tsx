...
const WebButton = ({ onPress, style, children }: any) => {
  if (Platform.OS === 'web') {
    return (
      <div onClick={onPress} style={{ ...style, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        {children}
      </div>
    );
  }
...
export default function MembershipScreen() {
  ...
  const fetchData = async () => {
    try {
      const [subRes, clubRes] = await Promise.all([
        ...
        subscriptionsApi.getClubCount()
      ]);
      setSubscriptionInfo(subRes.data);
      setClubMembersCount(clubRes.data.count || 0);
    } catch (error) {
  ...
  const handleCancel = async () => {
    Alert.alert('Résilier l\'abonnement', 'Votre abonnement sera actif jusqu\'à la fin de la période. Continuer ?', [
      { text: 'Non', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            ...
            Alert.alert('Résilié', 'Abonnement résilié avec succès');
            fetchData();
          } catch (error: any) {
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]} edges={['top']}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
        ...
        <View style={[styles.clubCard, { backgroundColor: theme.card, borderColor: '#DC1B78' }]}>
          ...
          {!isClubMember && clubMembersCount < CLUB_MAX_MEMBERS && (
            <>
              ...
              <WebButton onPress={handleJoinClub} style={{ backgroundColor: '#DC1B78', paddingVertical: 14, borderRadius: 12, marginTop: spacing.sm }}>
        ...
        {/* Subscription Plans */}
        <Text style={[typography.h2, { color: theme.title, marginBottom: spacing.sm }]}>Formules Artiste-Entrepreneur</Text>
        ...
        {PLANS.map((plan) => {
          ...
          return (
            <View key={plan.id} style={[styles.planCard, { backgroundColor: theme.card, borderRadius: radius.xl }, isCurrent && { borderColor: plan.color, borderWidth: 2 }]}>
              {plan.popular && (
              ...
              <View style={styles.featuresContainer}>
                {plan.features.map((feature, idx) => (
                  <View key={idx} style={styles.featureRow}>
                    <Ionicons name={feature.included ? 'checkmark-circle' : 'close-circle'} size={18} color={feature.included ? theme.success : theme.textSecondary} />
                    <Text style={[typography.bodySmall, { color: feature.included ? theme.text : theme.textSecondary, marginLeft: spacing.sm }]}>{feature.text}</Text>
                  </View>
                ))}
              </View>

              {!isCurrent && (
                <WebButton onPress={() => handleSelectPlan(plan.id)} style={{ backgroundColor: plan.color, marginHorizontal: 16, marginBottom: 16, paddingVertical: 14, borderRadius: 12 }}>
...
const styles = StyleSheet.create({
  ...
  clubCard: { padding: spacing.lg, borderRadius: radius.xl, borderWidth: 2, marginBottom: spacing.lg },
  clubHeader: { flexDirection: 'row', alignItems: 'center', marginBottom: spacing.md },
  clubBadge: { paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6, marginRight: spacing.sm },
  memberBadge: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 12 },
  clubFeatures: { marginBottom: spacing.md },
  clubFeatureRow: { flexDirection: 'row', alignItems: 'center', marginBottom: spacing.sm },
  clubPriceRow: { flexDirection: 'row', alignItems: 'baseline', marginBottom: spacing.sm },
  divider: { height: 1, marginVertical: spacing.xl },
  planCard: { marginBottom: spacing.lg, overflow: 'hidden' },
  popularBadge: { position: 'absolute', top: 12, right: 12, paddingHorizontal: 10, paddingVertical: 4, borderRadius: 12, zIndex: 1 },
  popularText: { color: '#FFF', fontSize: 10, fontWeight: '700' },
  planHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', padding: spacing.lg, paddingBottom: spacing.md },
  currentBadge: { flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 10, paddingVertical: 4, borderRadius: 12 },
  ...
  cancelBtn: { alignItems: 'center', paddingVertical: spacing.lg, marginTop: spacing.md },
});
